...
| O-13 | Queue wait (`job.queueWait`) is measured as `job.ClaimedAt - job.RunAfter`, from timestamps already stamped by the capture/claim path, rather than adding a dedicated `enqueued_at` column | No schema change needed for a first cut. Known gap: `EfJobQueue.ClaimAsync` rewrites `run_after` to the lease expiry on claim, so in production this measures time since the *previous* lease's expiry, not the original enqueue time, on any job that was ever re-claimed. The isolated unit test (which seeds `RunAfter` directly) does not catch this. Revisit with a real `enqueued_at` column if queue-wait numbers ever need to be trusted for a retried job |
