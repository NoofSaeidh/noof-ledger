...
| O-11 | The Logs page's "Record to database from" control offers only Verbose, Debug and Information — never Warning or above | `/transactions/{id}/trace` and its stage summary are built from Information-level stage events (5001–5005, 5009); allowing Warning+ would silently empty the trace page for anyone who chose it |
