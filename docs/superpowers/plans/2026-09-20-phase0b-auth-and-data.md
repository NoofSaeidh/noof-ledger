...
- **`TreatWarningsAsErrors=true`.** Consequences that will bite:
  ...
  - `IDE0161` — **file-scoped namespaces are enforced by the compiler.** `dotnet ef migrations add` emits a block-scoped namespace in the migration `.cs` file, which **fails the build** until converted. Convert that one; do not disable the rule. Leave the generated `.Designer.cs` and `ModelSnapshot.cs` alone — their `// <auto-generated />` marker exempts them, and EF overwrites any edit on the next scaffold.
