...
- **Keep `CLAUDE.md` short.** It is read in full at the start of every session, so length is a tax on
  every single one. Anything that runs past a short paragraph belongs in its own document under
  `docs/`, linked from `CLAUDE.md` in one line. Prefer deleting a rule the code now enforces by itself: a
